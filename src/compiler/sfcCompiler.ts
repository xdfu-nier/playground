...
const defaultVueUrl = import.meta.env.PROD
  ? `${location.origin}/${import.meta.env.BASE_URL}/vue.runtime.esm-browser.js` // to be copied on build
...
async function hashId(filename: string) {
  const hash = SHA256(Utf8.parse(filename)); // 先转为 WordArray 再 hash
  const hashHex = hash.toString(); // 转为 hex 字符串
  return hashHex.slice(0, 8); // 截取前8位
}
